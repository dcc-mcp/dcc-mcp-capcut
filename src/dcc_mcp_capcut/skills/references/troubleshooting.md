...
| HTTP 503 from `/call` | The broker refused the action and returned an `{"error": ...}` body: usually no panel drained it within 30 s, or the payload omitted `action`. | The adapter reports the body's `error` text (up to 500 characters) instead of the bare status, so read the message rather than the status code — the panel-timeout row above is the common case. A caller that omits `action` must supply one. |
| `CapCut bridge returned HTTP <code>: <detail>` | The broker failed with a body that is not `{"error": ...}` JSON, or with no readable body at all. | The message carries the status plus the body text (up to 500 characters) or the HTTP reason; an empty detail means the broker sent no readable body. |
