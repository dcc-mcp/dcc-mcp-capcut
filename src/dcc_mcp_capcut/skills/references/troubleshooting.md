...
| `no CapCut executable found` (Windows; `skip` elsewhere) | CapCut Desktop is not installed. | Call `capcut-setup`'s `installation_plan`, then execute it with an operator-owned grant. |
