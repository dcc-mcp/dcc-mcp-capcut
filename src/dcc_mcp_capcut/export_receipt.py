...
# Actions that can produce a deliverable and therefore honour the opt-in flag.
# `get_export_status` is read-only, so the receipt is the only contract rule
# that ever applies to it.
EXPORT_RECEIPT_ACTIONS = frozenset(
    {"export_video", "export_thumbnail", "get_export_status", "build_vlog_demo"}
)
...
def _require_number(source: dict[str, Any], key: str, where: str) -> float:
    ...
    if not _is_number(value) or value <= 0:
        raise RuntimeError(f"{where} must carry {key} as a number > 0")
    return float(value)
...
def _validate_duration(receipt: dict[str, Any], where: str, timed: bool) -> None:
    ...
    if timed:
        if not _is_number(value) or value <= 0:
            raise RuntimeError(f"{where} must carry duration_sec as a number > 0 for timed media")
...
def validate_export_receipt(action: str, receipt: Any) -> dict[str, Any]:
    ...
    _require_text(receipt, "path", where)
    ...
    if not kinds & _PICTURE_STREAM_KINDS:
        raise RuntimeError(f"{where} has no video or image stream; a deliverable needs a picture")
