...
| HTTP 503 from `/call` | The broker refused the action and returned an `{"error": ...}` body: usually no panel drained it within 30 s, or the payload omitted `action`. | The adapter now surfaces that body verbatim, so read the message rather than the status code — the panel-timeout row above is the common case. A caller that omits `action` must supply one. |
